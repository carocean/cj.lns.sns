...
exports.flow = function(frame, circuit, plug, ctx) {
	...
	print(String.format('计算机：%s 正在关机',owner));
	var run = Runtime.getRuntime();
	//docker stop %s && docker rm %s是最安全的，但run.exec只认前面一条，后台的命令却当成了参数
	//如果是后台运行-d参数，stop只能停止容器，容器id仍在，因此还需要rm掉，实际上一行docker rm -f user1就可以了，但为了保证期间，先执行stop，再执行rm则不需要强制了
	//但是run.exec不能执行shell的连续命令，因此干脆docker rm -f xx算了
	var process = run.exec(String.format("docker rm -f %s",owner));
	...
	print(String.format('计算机：%s 已关机',owner));
}
